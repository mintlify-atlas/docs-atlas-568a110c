...
sidebarTitle: "RAG"
description: "Initialize the RAG pipeline, sync documents, run direct queries, and check system status via the /api/rag/ endpoints. All require JWT authentication."
---
...
<ParamField body="collect_metrics" type="boolean">
  When `true`, the system records `QueryMetrics` and `RAGASMetrics` records in the database for later analysis via the admin metrics endpoints.
</ParamField>
...
An enhanced chat endpoint that integrates the full RAG pipeline with conversation history support. Unlike `POST /api/chat/`, this endpoint accepts an explicit `history` array so you can pass conversation context from any source (not just stored DB messages). It also auto-creates a new conversation if `conversation_id` is omitted, and falls back gracefully to the basic chat handler if the RAG modules are unavailable. Requires JWT authentication.
...
<ParamField body="history" type="array">
  Optional explicit conversation history to pass to the LLM. Each element is an object with `role` (`"user"` or `"assistant"`) and `content` (string). When `conversation_id` is provided and `history` is omitted, the backend automatically loads the 6 most recent messages from that conversation.
</ParamField>

<ParamField body="use_rag" type="boolean" default="true">
  Set to `false` to bypass the RAG pipeline and use the basic keyword-based response generator. Automatically set to `false` for short greetings (under 30 characters) to avoid unnecessary retrieval.
</ParamField>
...
Returns a detailed diagnostic snapshot of the RAG pipeline by calling `pipeline.get_system_status()`. This is a more verbose version of `GET /api/rag/status/` that exposes internal component metadata. Requires JWT authentication (`AllowAny` in the source — same note as `/api/rag/query/`).
