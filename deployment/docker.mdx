---
title: "Deploy NISIRA Assistant with Docker Compose"
sidebarTitle: "Docker"
description: "Run NISIRA Assistant locally with docker-compose.yml (MySQL) or in production with docker-compose.production.yml (PostgreSQL + pgvector + Nginx reverse proxy)."
---
...
| `frontend` | `./frontend` (Dockerfile)            | Built with `REACT_APP_API_URL` arg |
...
Both Compose files mount directories on the host into the backend container so that uploaded documents and vector embeddings survive container restarts:
...
In production, these are managed as named Docker volumes (`rag_data`, `chroma_data`) rather than bind mounts:
